...
|---|---|---|---|
| `kind` | string | **required** | Tracker backend. `"linear"`, `"github"`, or `"jira"`. |
| `endpoint` | string | `https://api.linear.app/graphql` | GraphQL endpoint (Linear only) or Jira Cloud site base URL (**required for Jira**, e.g. `https://yourdomain.atlassian.net`). Omit for GitHub — the client uses the GitHub API automatically. |
| `api_key` | string | **required** | API token. Use `$VAR_NAME` to read from an environment variable (recommended). For Jira, this is an [Atlassian API token](https://id.atlassian.com/manage-profile/security/api-tokens), paired with `username`. |
| `username` | string | `""` | **Jira only, required.** Atlassian account email paired with `api_key` for HTTP Basic auth. Not a secret — safe to set as a plain value. |
| `project_slug` | string | **required** | For Linear: the project slug from your Linear URL (e.g. `"ENG"`). For GitHub: `"owner/repo"`. For Jira: the project key (e.g. `"PROJ"`). |
| `default_issue_type` | string | `"Task"` | **Jira only.** Issue type used when Itervox creates a follow-up issue. Must match an issue type configured in your Jira project. |
| `active_states` | string[] | `["Todo", "In Progress"]` | Issue states Itervox actively polls and dispatches agents for. For Jira, these are workflow status names (e.g. `"In Progress"`), matched literally — no JQL authoring needed. |
...
| `backlog_states` | string[] | `["Backlog"]` (Linear), `[]` (GitHub, Jira) | States shown as the leftmost column(s) on the Kanban board. Issues in backlog states are displayed but not dispatched. |
...
**Jira note:** Jira Cloud only (Jira Server/Data Center is not supported). State names must match your project's actual workflow status names exactly (case-insensitive). Comments are posted as plain-text Atlassian Document Format (ADF) — markdown formatting characters are not rendered. Branch tracking uses a plain marker comment (`itervox:branch:<name>`) since Jira's ADF format has no hidden-comment equivalent to GitHub's HTML-comment trick, so this comment is visible on the issue.
